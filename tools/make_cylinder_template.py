...
# Eddy3D plugin identity (Outdoor/CMP/CylinderDomainCMP.cs, manifest lib id).
...
OLD_DOCUMENT_GUID = "4b39559d-96a8-4ea0-bf82-2992eb521881"
...
def build_cylinder_chunk() -> str:
    """Serialize the Cylinder Domain component as Object index 14.
...
def main() -> None:
    ...
    # 1. Swap the Box Domain object (index 14) for the Cylinder Domain.
    start = text.find('            <chunk name="Object" index="14">')
    end = text.find('            <chunk name="Object" index="15">')
    if start < 0 or end <= start:
        sys.exit("error: could not locate Object index 14/15 boundaries")
    block = text[start:end]
    if "Box Domain" not in block:
        sys.exit("error: Object index 14 is not the Box Domain component")
    text = text[:start] + build_cylinder_chunk() + text[end:]
    ...
    # 3. Append the wind directions panel as a new object and bump the counts.
    text = replace_once(
        text,
        '<item name="ObjectCount" type_name="gh_int32" type_code="3">27</item>',
        '<item name="ObjectCount" type_name="gh_int32" type_code="3">28</item>',
        "ObjectCount item")
    text = replace_once(text, '<chunks count="27">', '<chunks count="28">',
                        "DefinitionObjects chunks count")
    last_obj = text.find('            <chunk name="Object" index="26">')
    if last_obj < 0:
        sys.exit("error: Object index 26 not found")
    closer = text.find('\n          </chunks>', last_obj)
    if closer < 0:
        sys.exit("error: DefinitionObjects closing tag not found")
    text = text[:closer + 1] + build_wind_panel_chunk(27) + text[closer + 1:]
    ...
    # 5. Fresh document identity.
    text = replace_once(text, OLD_DOCUMENT_GUID, DOCUMENT_GUID, "DocumentID")
    text = replace_once(
        text,
